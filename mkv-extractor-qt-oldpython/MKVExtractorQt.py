...
import pickle # Utilisé pour le fichier de sauvegarde
import hashlib # Utilisé pour la traduction des sous titres
from glob import glob # Utilisé pour lister le contenu d'un dossier
...
Variables = {"Cmd" : "", # Commande à éxecuter
             ...
             "ConfigFile" : os.path.join(QDir.homePath(), '.config/MKV-Extractor-Qt/config'), # Fichier de configuration
             "ConfigFolder" : os.path.join(QDir.homePath(), '.config/MKV-Extractor-Qt'), # Dossier de configuration
...
if not os.path.exists(Variables["ConfigFolder"]):
    os.makedirs(Variables["ConfigFolder"])

# Si le dossier et le fichier existent, essaie de charger le dictionnaire de configs
elif os.path.exists(Variables["ConfigFile"]): # Verifie l'existence du fichier de config
    with open(Variables["ConfigFile"], 'rb') as ConfigFile: # Ouverture avec fermeture auto du fichier de config
        mon_depickler = pickle.Unpickler(ConfigFile)

        try: # Verifie que le chargement des configs est possible (déjà eu 1 soucis avec un fichier corrompu)
            Configs = mon_depickler.load()

        except: # Le fichier de config n'est pas bon mais un dico de base sera cré juste apres
            pass

# Teste l'appel à la dernire variable ajoutée au dictionnaire de configs (si pas de fichier de configs ou si plantage en chargement)
try:
    Configs["SplitterState"]

# Si la variable n'existe pas, création d'une version basique du dico
except:
    Configs = {"Ac3Kbits" : 128, # Qualité des fichiers ac3 convertis
               ...
               "Geometry" : "", # Option de sauvegarde la resolution de la fenêtre
               "MKVDirNameIn" : "", # Dossier du fichier mkv d'entrée : Path(MKVLink).name
               "MKVDirNameOut" : "", # Dossier de sortie
               ...
               "SplitterState" : None, # Permet de sauvegarder et de charger l'aspect du splitter
...
def IconBis(icon):
    ...
    Icon2 = QIcon()
    Icon2.addPixmap(QPixmap(":/img/{}.png".format(icon)), QIcon.Normal, QIcon.Off) # Icone secondaire des ressources
    return QIcon.fromTheme(icon, Icon2)
...
class MKVExtractorQt(QMainWindow):
    def __init__(self, parent=None):
        ...
        if Configs["Geometry"]:
            self.resize(Configs["Geometry"][0], Configs["Geometry"][1]) # Resize de la fenetre si les valeurs ont été save

        # Aspect du splitter
        if Configs["SplitterState"]:
            self.ui.splitter.restoreState(Configs["SplitterState"])
        ...
        if not WhichLorraine("ffmpeg") and not WhichLorraine("avconv"):
            ### Cache les radiobutton et l'option de conversion
            self.ui.option_dts_ac3.setVisible(False)

        elif WhichLorraine("ffmpeg") and not WhichLorraine("avconv"):
            ### Cache les radiobutton et selectionne automatiquement ffmpeg
            Configs["FFMpeg"] = True

        elif not WhichLorraine("ffmpeg") and WhichLorraine("avconv"):
            ### Cache les radiobutton et selectionne automatiquement ffmpeg
        ...
        if ffconv:
            self.option_ffmpeg = QAction(IconBis("ffmpeg"), '', self, checkable=True) # Creation d'un item sans texte
        ...
        self.option_stereo = QAction(IconBis("audio-headphones"), '', self, checkable=True) # Creation d'un item sans texte
        ...
        self.option_del_temp = QAction(IconBis("edit-delete"), '', self, checkable=True) # Creation d'une action cochable sans texte
        self.option_subtitles_open = QAction(IconBis("document-edit"), '', self, checkable=True) # Creation d'une action cochable sans texte
        menulist = QMenu() # Creation d'un menu
        ...
        if Configs["SameFolder"]: # Coche la case si l'option est true
            ...
            # L'option SameFolder étant coché, il est inutile de garder en mémoire le dossier de sortie
            if Configs["MKVDirNameOut"]:
                Configs["MKVDirNameOut"] = ""

        if not os.path.exists(Configs["MKVDirNameOut"]): # Réinitialisation du dossier de sortie s'il n'existe pas
            Configs["MKVDirNameOut"] = ""
        ...
        self.ui.mkv_tracks.setColumnWidth(1, 25) # Definit la colonne 1 à 25px
        self.ui.mkv_tracks.setColumnWidth(2, 25) # Definit la colonne 2 à 25px
        self.ui.mkv_tracks.setColumnWidth(3, 25) # Definit la colonne 3 à 25px
    ...
    def connectActions(self):
        ...
        self.ui.mkv_info.activated.connect(self.MKVInfoGui) # Au clic sur le menu
        self.ui.mkv_view.activated.connect(self.MKVView) # Au clic sur le menu
        self.ui.mkv_mkvmerge.activated.connect(self.MKVMergeGui) # Au clic sur le menu
        self.ui.mk_validator.activated.connect(self.MKValidator) # Au clic sur le menu
        self.ui.mk_clean.activated.connect(self.MKClean) # Au clic sur le menu
    ...
    def CheckSize(self):
        ...
        filesize = os.path.getsize(Variables["MKVLinkIn"])

        # En fonction de la version python
        st = os.statvfs(Configs["MKVDirNameOut"])
        disksize = st.f_bavail * st.f_frsize # Récupération de la place disponible
    ...
    def OutInfo(self):
        ...
        file = os.path.join(QDir.homePath(), 'InfoMKVExtractorQt.txt')
...
# Fenetre de séléction de sortie du fichier mkv
        ...
        MKCleanTemp = QFileDialog.getSaveFileName(self, self.Trad["SelectFileOut"], QDir.path(QDir(Configs["MKVDirNameOut"] + "/clean_" + Configs['MKVFileNameIn'])), "Matroska File (*.mkv)")
...
# Préparation de la commande
        Variables["Cmd"] = ["MKValidator", "", 'mkvalidator "{}"'.format(Variables['MKVLinkIn'])]
...
# Lancement de la commande
    ...
    def OptionsValue(self, option, value):
        ...
        if option == "SameFolder" and value:
            if Variables["MKVLinkIn"]: # Vérifie qu'un fichier a déjà été chargé
                ...
                self.ui.mkv_folder.setStatusTip(self.Trad["SelectedFolder"].format(Configs["MKVDirNameOut"]))
                self.SetInfo(self.Trad["SelectedFolder2"].format(Configs["MKVDirNameOut"]), newline=True)
    ...
    def ViewInfo(self, value):
        ...
        # Affiche ou cache la box
        self.ui.reply_info.setVisible(value)

        # Hauteur du splitter
        height = self.ui.splitter.geometry().height()

        if value:
            self.ui.reply_info_box.setMinimumSize(QSize(0, 100))
            self.ui.splitter.setSizes([100, height - 100])
        else:
            self.ui.reply_info_box.setMinimumSize(QSize(0, 40))
            self.ui.splitter.setSizes([40, height - 40])
...
# Recharge les textes de l'application graphique
...
# Si un fichier à déjà été chargé
...
# Si un dossier de sortie a déjà été séléctionné
    ...
    def MKVFolder(self, MKVFolderTemp=None):
        ...
        if not MKVFolderTemp:
            MKVFolderTemp = QFileDialog.getExistingDirectory(self, self.Trad["SelectFolder"], QDir.path(QDir(Configs["MKVDirNameOut"])))

        # Continuation de la fonction si un fichier est bien séléctionné
        if os.path.isdir(MKVFolderTemp):
            ...
            # Les widgets n'ont pas été debloqué avant car pas de dossier de sortie de donnée
            if Variables["MKVLoad"] and MKVDicoSelect:
                ...
                for valeurs in MKVDicoSelect.values(): # Boucle sur la liste des lignes
                    if "dts" in valeurs: # Recherche la valeur dts dans les sous listes
                        self.ui.option_dts_ac3.setEnabled(True) # Deblocage du widget

                    elif "sub" in valeurs: # Recherche la valeur vobsub dans les sous listes
    ...
    def MKVOpen(self, MKVLinkTemp=None):
        ...
        if not MKVLinkTemp:
            MKVLinkTemp = QFileDialog.getOpenFileName(self, self.Trad["SelectFileIn"], QDir.path(QDir(str(Configs["MKVDirNameIn"]))), "Matroska Files(*.mka *.mks *.mkv *.mk3d *.webm *.webmv *.webma)")

        # Continuation de la fonction si un fichier est bien séléctionné
        if os.path.exists(MKVLinkTemp):
            Variables["MKVLinkIn"] = MKVLinkTemp # Mise à jour de la variable
            Variables["MKVFileNameIn"] = os.path.basename(MKVLinkTemp) # Mise à jour de la variable
            Configs["MKVDirNameIn"] = os.path.dirname(MKVLinkTemp) # Mise à jour de la variable
...
# Mise à jour de la variable indiquant qu'un fichier à été chargé
        Variables["MKVLoad"] = True
        Variables["MKVTime"] = 0
        Variables["MKVTitle"] = ""
        Variables["Tracks"] = []
...
# (Re)Creation d'un dossier temporaire qui s'effacera tout seul lors de la destruction de la variable
        Variables["TempFolder"] = TemporaryDirectory(prefix="MKV-Extractor-Qt-")
        Variables["TempFolderName"] = Variables["TempFolder"].name # Récupération du nom du dossier
        os.makedirs(os.path.join(Variables["TempFolderName"], "SUB2SRT")) # Création du dossier de conversion des sous titres
...
# Nettoyage de la liste de piste
        for val in list(range(len(Variables["Tracks"]))):
...
# Boucle traitant les pistes du fichier mkv
    ...
    # Traitement des pistes normales
        ...
        # Traitement des pistes normales
            ...
            # Envoie des informations dans le tableaux
                    ...
                    self.ui.mkv_tracks.setItem(x, 2, QTableWidgetItem(IconBis(icone),"")) # Envoie de l'icône
                    self.ui.mkv_tracks.item(x, 2).setFlags(Qt.NoItemFlags | Qt.ItemIsEnabled) # Blocage de la modification
                    self.ui.mkv_tracks.item(x, 2).setStatusTip(self.Trad["TrackType"].format(TrackType)) # StatusTip
                    self.ui.mkv_tracks.setItem(x, 3, QTableWidgetItem(IconBis("layer-visible-off"),"")) # Envoie de l'icone de visualisation
    ...
    # Traitement spécifique aux fichiers joints
        ...
        # Envoie des informations dans le tableaux
                ...
                self.ui.mkv_tracks.setItem(x, 3, QTableWidgetItem(IconBis("layer-visible-on"),"")) # Envoie de l'icone de visualisation
...
# Dans le cas d'un clic sur l'icone avec l'oeil = colonne 3
        if y == 3:
    # Ne traiter que les icones on
            if MKVDico[x][3] == "layer-visible-on":

        # Dans le cas d'un fichier de chapitrage
                if MKVDico[x][1] == "Chapters":
                    fichier = os.path.join(Variables["TempFolderName"], "chapters.txt") # Fichier de sortie
                    Variables["MKVChapters"] = fichier

            # Ne pas extraire si le fichier existe deja
        ...
        # Dans le cas de global tags
                    ...
                    fichier = os.path.join(Variables["TempFolderName"], "tags.xml") # Fichier de sortie
                    Variables["MKVTags"] = fichier
        ...
        # Dans le cas de fichier joint
                    ...
                    fichier = os.path.join(Variables["TempFolderName"], 'attachement_{0[0]}_{0[4]}'.format(MKVDico[x])) # Fichier de sortie
...
# Récupération de la cellule modifiée
        x = self.ui.mkv_tracks.row(info) # Récupération de l'emplacement de l'item modifié
        y = self.ui.mkv_tracks.column(info) # Récupération de l'emplacement de l'item modifié

# Dans le cas de la modification d'une checkbox
        if y == 1:
    # Modifications graphiques
            for widget in [self.ui.option_dts_ac3, self.ui.option_vobsub_srt, self.ui.option_reencapsulate, self.ui.mkv_execute]:
                widget.setEnabled(False)

    # Mise au propre du tableau
            MKVDicoSelect.clear() # Mise au propre du dictionnaire
            ligne = 0
            while ligne < self.ui.mkv_tracks.rowCount(): # Boucle traitant toutes les lignes du tableau
                if self.ui.mkv_tracks.item(ligne, 1).checkState(): # Teste si la ligne est cochée
                    MKVDicoSelect[ligne] = MKVDico[ligne]
                ligne += 1
    ...
    # Arret de la fonction si pas de séléction ou pas de dossier de sortie
            if not MKVDicoSelect or not Configs["MKVDirNameOut"]:
                return
...
# Dans le cas d'une modification de texte
    ...
    # Mise à jour du texte dans le dico des pistes
            CellText = self.ui.mkv_tracks.item(x, y).text() # Récupération du nouveau texte de la cellule
            MKVDico[x][y] = CellText
    ...
    # Mise à jour du texte dans le dico des pistes séléctionnées
            if self.ui.mkv_tracks.item(x, 1).checkState(): # Verifie si la ligne est séléctionnée
                MKVDicoSelect[x][y] = CellText


    #========================================================================
    def MKVExecute(self):
        """Fonction créant tous les commandes : mkvextractor, ffmpeg, mkvmerge..."""
# Mise au propre et initialisation de variables
        Variables["CmdOld"] = "" # Garde en mémoir ele nom du dernier pack de commande lancé
        ...
        Variables["MKVLinkOut"] = "" # Fichier mkv de sortie en cas de réencapsulage
        Variables["MKVChapters"] = "" # Fichier de sortie des chapitres
        Variables["MKVTags"] = "" # Fichier de sortie des tags
...
# Boucle traitant les pîstes une à une
...
# Traitement des pistes videos, maj de commandes
            if Select[2] == "video-x-generic":
                Variables["TempFiles"].append("{0}/{1[0]}_video_{1[4]}.mkv".format(Configs["MKVDirNameOut"], Select))
...
# Dans le cas où il y aura conversion des dts en ac3, maj de commandes
                if Variables["DtsToAc3"]:
                    Variables["TempFiles"].append("{0}/{1[0]}_audio_{1[4]}.ac3".format(Configs["MKVDirNameOut"], Select))
...
# Dans le cas où il n'y aura pas de conversion des dts en ac3, maj de commandes
                    ...
                    Variables["TempFiles"].append("{0}/{1[0]}_audio_{1[4]}.{1[6]}".format(Configs["MKVDirNameOut"], Select))
...
# Dans le cas de sous titres de fichiers sub
                if Select[6] == "sub":
                    Variables["TempFiles"].append("{0}/{1[0]}_sous-titres_{1[4]}.idx".format(Configs["MKVDirNameOut"], Select))
                    Variables["TempFiles"].append("{0}/{1[0]}_sous-titres_{1[4]}.sub".format(Configs["MKVDirNameOut"], Select))
...
# Dans le cas d'un conversion SUB => AC3, maj de commandes
                    if Variables["VobsubToSrt"]:
                        Variables["TempFiles"].append("{0}/{1[0]}_sous-titres_{1[4]}.srt".format(Configs["MKVDirNameOut"], Select))
...
# Dans le cas de sous titres autre que de type sub, maj de commandes
                    ...
                    Variables["TempFiles"].append("{0}/{1[0]}_sous-titres_{1[4]}.{1[6]}".format(Configs["MKVDirNameOut"], Select))
...
# Traitement des pistes chapitrage, maj de commandes
                ...
                Variables["TempFiles"].append("{}/chapters.txt".format(Configs["MKVDirNameOut"]))
                ...
                Variables["MKVChapters"] = os.path.join(Configs["MKVDirNameOut"], "chapters.txt")


# Traitement des pistes de tags, maj de commandes
            elif Select[2] == "text-html":
                Variables["TempFiles"].append("{}/tags.xml".format(Configs["MKVDirNameOut"]))

                mkvmerge += '--global-tags "{}/tags.xml" '.format(Configs["MKVDirNameOut"])

                mkvextract_tag = 'mkvextract tags "{}" '.format(Variables["MKVLinkIn"])

                Variables["MKVTags"] = os.path.join(Configs["MKVDirNameOut"], "tags.xml")
...
# Traitement des pistes jointes, maj de commandes
                ...
                Variables["TempFiles"].append("{0}/{1[0]}_{1[4]}".format(Configs["MKVDirNameOut"], Select))
...
# Ajout de la commande mkvextract_tag à la liste des commandes à executer
        if mkvextract_tag and not os.path.exists(Variables["MKVChapters"]):
...
# Ajout de la commande mkvextract_chap à la liste des commandes à executer
        if mkvextract_chap and not os.path.exists(Variables["MKVTags"]):
...
# Fenetre de séléction de sortie du fichier mkv
            Variables["MKVLinkOut"] = QFileDialog.getSaveFileName(self, self.Trad["SelectFileOut"], QDir.path(QDir(Configs["MKVDirNameOut"] + "/MEG_" + Variables["MKVFileNameIn"])), "Matroska File (*.mkv)")

# Dans le cas où aucun fichier n'est choisi
            if not Variables["MKVLinkOut"]:
                return
...
# Dans le cas où il faut réencapsuler
            Variables["MKVTitle"] = self.ui.mkv_title.text() # Récupération du titre du fichier, s'il est vide, il plante mkvmerge
...
# Dans le cas où un travail est lancé
        if value:
...
# Si variable stop activée, arret brutal du travail en cours
        ...
        data = self.process.readAllStandardOutput() # Récupération du retour (les 2 sorties sont sur la standard)

        for line in bytes(data).decode('utf-8').splitlines(): # Récupération du texte retourné
# Passe la boucle si le retour est vide, ce qui arrive et provoque une erreur apres
            if line == "":
                continue # Relance la boucle

# Traitement des retours de MKVMerge, Ne rien faire pour : listage des langues, listage des pistes, Open SRT
                ...
                if Variables["Cmd"][1] in ["LanguagesList", "TracksList"]:
                    if Variables["Cmd"][1] == "LanguagesList": # Dans le cas du listage des langues
                        ...
                        if line[0] in "ABCDEFGHIJKLMNOPQRSTUVWXYZ": # Si le 1er caractere est une majuscule, on ajoute la piste
                            Variables["Tracks"].append(line)

                        else: # Sinon, c'est que les lignes sont découpées, on conserve donc la valeur en memoire
                            Variables["Tracks"][-1] = Variables["Tracks"][-1] + line

                    continue # Relance la boucle pour récuper les valeurs suivantes


                elif Variables["Cmd"][1] == "Merge": # Dans le cas d'un encapsulation
                    if line[-1] == "%": # Traiter le retour en cas de presence de pourcentage
                        line = int(line.split(": ")[1].strip()[0:-1]) # Récupere le nombre


# MKVInfo sert à écupérer quelques infos sur le fichier mkv
            elif Variables["Cmd"][0] == "MKVInfo":
                if "+ Title:" in line: # Récupération du titre du fichier MKV
                    Variables["MKVTitle"] = line.split(": ")[1]

                elif "+ Duration:" in line: # Récupération de la durée du fichier MKV
                    Variables["MKVTime"] = int(line.split(": ")[1].split(".")[0])

                elif "+ Track number:" in line: # Récupération de la numero de la piste pour être utilisé pour les fps ci après
                    Variables["TempInfo"] = int(line.split(": ")[-1].split(")")[0])

                elif "+ Default duration:" in line: # Récupération de la durée du fichier MKV
...
# MKVExtract renvoie une progression ou les contenus des fichiers de tags et chapitres. Les fichiers joints ne renvoient rien.
                ...
                if Variables["Cmd"][1] == "Tracks": # Dans le cas de l'extraction de pistes simples
                    if line[-1] == "%": # Traiter le retour en cas de presence de pourcentage
                        line = int(line.split(": ")[1].strip()[0:-1]) # Récupere le nombre

                else:
                    if Variables["Cmd"][1] == "Chapters": # En cas d'extraction de chapitres
                        with open(Variables["MKVChapters"], 'a') as ChaptersFile:
                            ChaptersFile.write(line+'\n')

                    elif Variables["Cmd"][1] == "Tags": # En cas d'extraction de tags
                        with open(Variables["MKVTags"], 'a') as TagsFile:
                            TagsFile.write(line+'\n')

                    continue # Relance la boucle pour récuper les valeurs suivantes


# MKValidator ne renvoie pas de poucentage mais des infos ou des points
...
# FFMpeg ne renvoie pas de pourcentage mais la durée de vidéeo encodée en autre
            elif Variables["Cmd"][0] in ["ffmpeg", "avconv"]:
                if "time=" in line and Variables["MKVTime"]: # Pourcentage maison se basant sur la durée du fichier
...
# Sub2Srt est en fait un ensemble de commande mais seul Tesseract à besoin d'être traité
                ...
                if Variables["Cmd"][1] == "TesseractLang": # Récupération des langues (3 carac) dispo dans Tesseract
                    if len(line) == 3:
                        TesseractLanguages.append(line) # Ajout de la langue

                    continue # Relance la boucle pour récuper les valeurs suivantes

                elif Variables["Cmd"][1] == "Tesseract": # Pourcentage maison se basant sur le nombre de fichier fait et restant
...
# Affichage du texte ou de la progression si c'est une nouvelle valeur
            if line and line != Variables["WorkOldLine"]: # Comparaison anti doublon
                ...
                if type(line) is int: # Envoie du pourcentage à la barre de progression si c'est un nombre
                    self.ui.progressBar.setValue(line)

                else: # Envoie de l'info à la boite de texte si c'est du texte
                    self.SetInfo(line)
...
# Si le travail est annulé (via le bouton stop ou via la fermeture du logiciel) ou a renvoyée une erreur
        if Variables["WorkStop"] or self.process.exitCode() != 0:
            ...
            if Variables["WorkStop"]: # Envoie du texte le plus adapté
...
# La 1ere commande sert à mettre les bases du systeme
        if Variables["Cmd"][1] == "Demarrage":
            ...
            for file in glob(os.path.join(Folder, "*")): # Suppression des fichiers du dossier de conversion
                os.remove(file)

            with open(IDX, "r") as fichier_idx: # Lit le fichier idx original ligne par ligne et renvoie le tout (ligne + la ligne modifiée) dans un nouveau fichier
                with open(IDXNew, "w") as new_fichier_idx:
            ...
            os.remove(IDX) # Suppression du fichier idx original
...
# La 2e commande est la transformation sub en images
        elif Variables["Cmd"][1] == "Subp2Tiff":
            Variables["ImgFiles"] = []
            for File in ["*.pgm", "*.tif"]: # Creation d'une liste des fichiers sous titres
                Variables["ImgFiles"].extend(glob(os.path.join(Folder, File)))

            Variables["ImgFilesNb"] = len(Variables["ImgFiles"]) # Récupération du nombre totale de sous titre à traiter
...
# La 3e commande est la reconnaissance des images en textes
            ...
            if Variables["ImgFiles"]: # Boucle qui traite les images une à une tant qu'il y en a
            ...
            else: # un fois le travail de la boucle terminé
                MD5Dico.clear() # Dictionnaire listant les sous titres à travailler

                for File in glob(os.path.join(Variables["TempFolderName"], "SUB2SRT", "*.txt")): # Pour chaque fichier txt
                    if os.path.getsize(File) == 0: # Si le fichier est vide 0ko
                        image = File[:-4] # Récupere le nom du fichier image (enleve l'extension .txt)
                        hash = hashlib.md5(open(image, "rb").read()).hexdigest() # Hash le fichier

                        if hash in MD5Dico.keys(): # Si le hash existe deja, c'est une image doublon
                            if not image in MD5Dico[hash]: # Si le fichier n'est pas deja dans la liste
                                MD5Dico[hash].append(image) # Ajout du fichier a la liste en lien avec le hash
                        else: # Si le hash n'existe pas
                            MD5Dico[hash] = [image] # On ajoute une nouvelle paire hash : fichier

                if MD5Dico: # Si le dico contient des fichiers à reconnaitre manuellement
                    ...
                    self.IMGViewer(0) # Appelle de la fonction qui affiche le texte et l'image
                ...
                else: # S'il n'y pas de fichier à reconnaitre manuellement
...
# La 4e commande est la création d'un fichier srt depuis les reconnaissances : SubpTools
    ...
    def RemoveTempFiles(self):
        ...
        for file in Variables["TempFiles"]: # Boucle traitant chaque fichier temporaire
            if os.path.exists(file): # Si le fichier existe
                os.remove(file) # SUppression du fichier
    ...
    def WorkStop(self):
        ...
        if Variables["WorkPause"]: # Dans le cas ou le logiciel est en pause
            self.WorkPause() # La pause est levé, le travail relancé mais va s'arreter avec avec la variable WorkStop


    #========================================================================
    def WorkPause(self):
        """Fonction d'arret du travail en cours, met simplement à jour une variable d'arret."""
        Variables["WorkPause"] = False # Deblocage de la pause
        self.ui.mkv_resume.hide() # Cache le bouton de reprise
    ...
    def IMGViewer(self, change):
        ...
        md5 = list(MD5Dico.keys())[Variables["SubNum"]] # Récupération d'une clé

        img = MD5Dico[md5][0] # Selectionne la 1ere image de la clée
        txt = "{}.txt".format(img)
        self.ui.image_viewer.setPixmap(QPixmap(img)) # Affichage de l'image

        ### Progression du travail
        progression = int(((Variables["SubNum"] + 1) * 100) / Variables["SubNb"]) # Progression du travail
        ...
        ### Modifications graphiques
        self.SetInfo("Edition des fichiers ayant le md5 : {}".format(md5))
        ...
        if os.path.getsize(txt) > 0: # Si le fichier texte n'est plus vide (en cas de retour en arriere)
            with open(txt, "r") as SubFile:
                text = SubFile.read()
    ...
    #========================================================================
    def TextUpdate(self):
        SubText = self.ui.sub_text.toPlainText() # Récupération du texte
        md5 = list(MD5Dico.keys())[Variables["SubNum"]] # Récupération de la clé

        if SubText: # Si le texte n'est pas vide, on met à jour les fichiers txt
            for ImgFile in MD5Dico[md5]:
                TxtFile = "{}.txt".format(ImgFile)
                with open(TxtFile, "w") as SubFile:
    ...
    def dragEnterEvent(self, event):
        ...
        file = event.mimeData().text()[7:] # Récupération du nom du fichier

        if os.path.isfile(file): # En cas de fichier (pour le fichier d'entrée)
            if file.split(".")[-1] in ["mka", "mks", "mkv", "mk3d", "webm", "webmv", "webma"]: # Vérifie que l'extension
                event.accept()

        elif os.path.isdir(file): # En cas de dossier (pour le dossier de sortie)
            event.accept()
    ...
    def closeEvent(self, event):
        """Fonction lancée à la fermeture de la fenêtre quelqu'en soit la méthode."""
        # Arret du travail en cours
        Variables["WorkStop"] = True
        if not self.process.waitForFinished(3000): # Attend que le travail soit arrété pdt 3s
            self.process.kill()

        # Si l'option de suppression de confgi est activé, on supprime le fichier et on ne save pas les valeurs
        if Configs["DelConfig"]:
            if os.path.exists(Variables["ConfigFile"]):
                os.remove(Variables["ConfigFile"])

        # Dans le cas normal, les valeurs sont saves
        else:
            Configs["Geometry"] = [self.geometry().width(), self.geometry().height()]
            Configs["WinMax"] = self.isMaximized()

            # Sauvegarde des infos
            with open(Variables["ConfigFile"], 'wb') as ConfigFile: # Enregistrement du fichier configs
                mon_pickler = pickle.Pickler(ConfigFile)
                mon_pickler.dump(Configs)
